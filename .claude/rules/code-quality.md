...
- **Modals**: `FormModal` (`internal/tui/form_modal.go`) for forms; any hand-built overlay panel needs `panel.Box = tview.NewBox()` first or the layer beneath bleeds through. A new modal implements `HandleKey` and `Focus` and registers in `modalBindings` (`internal/tui/modal_dispatch.go`); never add a branch to `handleGlobalKey`.
- **Form fields**: `AddInput`, `AddTextArea`, `AddPicker`, `AddPackedInput`, `AddSplitRow`, `AddCheckbox`, `AddStatic`. Pickers are `FormPicker` (`form_picker.go`), not `tview.DropDown`, which sizes its open menu to the option count with no cap. Tab and Backtab move between fields: arrows belong to the focused widget, so never bind them to navigation.
