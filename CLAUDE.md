...
`issueFieldOptions` in `pickers.go` is the matching loader for the fields the overlay picker and the inline chooser both serve: state, assignee, priority, project, milestone, cycle. A new field of that kind needs both a constructor and a case. Text fields and the multi-select have constructors only. Every overlay goes through `ShowFieldPicker`, and the chosen `PickerItem` carries its own `Name` so a save message prints "Cycle 12" where the row read "Cycle 12 (active)".
...
Four rules there. The render **clamps `choice` and `offset` itself**, because it runs inside `Draw` and an index past a list that came back shorter is a panic in a frame. Every move re-renders and then **scrolls the lit row, not the block** — `refitDetailsPage` early-returns on an unchanged width, so nothing else will draw the highlight, and the wheel goes straight through the pane, so the chooser can be scrolled off. Both ways out end in `scrollFieldIntoView`, or the rows going away leave the field above the viewport. And the load's generation counter is `App.chooserGeneration`, **not** a field of `detailsEditState`, which every exit zeroes: a counter inside it restarts at nought and the next opening collides with the last one's in-flight load.
