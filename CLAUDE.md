...
**The keys reference is the one surface that has to name all of them.** `keyContext()` (`key_context.go`) is where the keyboard is, at one value per handler `handleGlobalKey` routes to, read in that same routing order; `keySections` (`keys_modal.go`) is a `rows func(*App) []hint` per context, resolved at open time so a settings save rebinds what it prints, and dropping a row whose key came back empty the way `hintLine` does. `updateStatusBar` is deliberately **not** on it: its switch is finer, splitting a zoomed pane from an unzoomed one, because those change what one line should say. `show_keys` is a command rather than a UI action, so it stays in the palette when a binding takes `?`; the two default-deny handlers that never reach the shortcut dispatch (`handleDetailsEditKey`, `handleChooserKey`) read `keysReferenceKey()` rather than a literal. `TestEveryHandlerKeyIsListed` parses the package and reads each handler's `case` clauses back out, so a context that gains a key the reference does not name fails there; `keysNotListed` is the exemptions, each with its reason.
