...
**One mouse capture owns the pane.** `handleMouse` (`mouse_focus.go`) hit-tests a left press against the panes `contentFlex` mounts and sets `focusedPane` before tview delivers the click, so the press lands on the widget and the pane agrees with it. Three rules there. It walks the mounted items rather than the three panes' rects, since `Flex.Clear` leaves an unmounted pane's rect where the last draw put it. It swallows a press that reflowed the layout, because the rects are a frame behind and forwarding hands the click to whatever moved in under the pointer. And it redraws itself: tview paints after a mouse event only when a primitive consumed it, and a press on a pane's border consumes nothing. Sub-focus stays with the controls — `claimNavFocus` for the query box and the tree, `enterCommentsFocus` for the writing boxes.
