...
`commentBlocks` (`comment_thread.go`) is the page's comment order: threads from `buildCommentRows`, the reply box at the end of the thread it answers, the compose card last. The ring `{` and `}` walk is the `commentSpans` the same render recorded, so **what the braces do follows what was drawn** — a stop that is not on the page cannot be focused. Inside a writing box the braces are prose; Esc is the way back to the cards. Linear rejects a `parentId` that is not top level, so a reply always posts against `threadRootID`.
