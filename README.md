...
- Optional grouping and subgrouping (status, priority, assignee, cycle) like Linear's list view — `group_by` and `subgroup_by` in `config.json`, or the `Group issues by…` and `Subgroup issues by…` commands
