...
// Save writes the session file. The write is atomic: an interrupted in-place
// write would leave JSON that Load rejects, and the next Record would then
// rebuild from empty, costing every workspace its saved place rather than just
// the one being written. Mode 0600 matches the credentials store, since the
// file holds the user's search text and the issues they were reading.
